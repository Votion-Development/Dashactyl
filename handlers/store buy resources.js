...
module.exports.load = async function(app, ifValidAPI, ejs) {
    ...
    async (req, res) => {
        ...
        let redirects = process.pagesettings.redirectactions.buy_resources;
        if (!req.session.data || !req.session.data.userinfo) return functions.doRedirect(req, res, redirects.notsignedin);

        let current = await process.db.fetchAccountDiscordID(req.session.data.userinfo.id);
            ...
            if (process.env.store[type].enabled) {
                ...
                if (typeof amount == "object" && amount == null) {
                    ...
                    if (amount < 0) {
                        ...
                        let check_multiple = amount / process.env.store[type].per;
