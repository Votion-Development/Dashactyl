...
module.exports.load = async function (app, ifValidAPI, ejs) {
  ...
  for (const { server_id, date, action } of (await process.db.getAllRenewalTimers())) {
    ...
    if (time_left <= 0) {
      if (action === 'suspend' || action === 'auto') {
        ...
        if (action === 'auto') {
          ...
          const user_info = await process.db.fetchAccountPterodactylID(server_info.attributes.user)
